...
SOURCE = ROOT / "thesis_docs" / "final_manuscript_complete" / "Finals_Complete_Paper_WASD.md"
OUTPUT = ROOT / "thesis_docs" / "final_manuscript_complete" / "Finals_Complete_Paper_WASD.docx"
...
def add_inline(paragraph, text, template=None):
    ...
    for match in pattern.finditer(text):
        ...
        if token.startswith("__*") and token.endswith("*__"):
            add_styled_run(paragraph, token[3:-3], template, bold=True, italic=True)
            ...
            if m:
                ...
                add_styled_run(paragraph, token, template)
        else:
...
def table_widths(column_count):
    ...
    if column_count == 3:
        # Table 3.2
        return [Inches(1.8), Inches(3.4), Inches(1.3)]
    if column_count == 4:
        # Table 3.3
        return [Inches(1.55), Inches(1.55), Inches(2.2), Inches(1.2)]
    if column_count == 6:
        # Table 4.2 (Comparison, Delta PR-AUC, Delta F0.5, Delta Precision, Delta Recall, Sig?)
        return [Inches(1.7), Inches(1.0), Inches(0.95), Inches(0.95), Inches(0.95), Inches(0.95)]
    if column_count == 9:
        # Table 4.3 (Model, Val PR-AUC, Test PR-AUC, Val F0.5, Test F0.5, Val Prec, Test Prec, Val Rec, Test Rec)
        return [Inches(1.3), Inches(0.65), Inches(0.65), Inches(0.65), Inches(0.65), Inches(0.65), Inches(0.65), Inches(0.65), Inches(0.65)]
    if column_count == 12:
        # Table 4.1 (Model, Input, Test PR-AUC, Test ROC-AUC, Test F0.5, Precision, Recall, Specificity, TP, FP, FN, TN)
        return [
            Inches(1.1), Inches(1.1), Inches(0.65), Inches(0.65), Inches(0.55),
            Inches(0.42), Inches(0.38), Inches(0.42), Inches(0.28), Inches(0.28),
...
def add_table(document, rows, table_template):
    ...
    font_size = Pt(8.0) if col_count >= 12 else (Pt(8.5) if col_count >= 6 else Pt(9.5))
    ...
    for row_index, row in enumerate(rows):
        for column_index, value in enumerate(row):
            ...
            paragraph.alignment = WD_ALIGN_PARAGRAPH.CENTER if (row_index == 0 or column_index >= 2) else WD_ALIGN_PARAGRAPH.LEFT
            paragraph.paragraph_format.space_after = Pt(1)
            paragraph.paragraph_format.space_before = Pt(1)
            paragraph.paragraph_format.line_spacing = 1.0
            
            clean_text = math_text(value)
            is_bold = row_index == 0 or "**" in value or "__" in value
...
def main():
    ...
    templates = {
        "cover_title": source_paragraphs[0],
        "cover_line": source_paragraphs[3],
        "cover_school": source_paragraphs[4],
        "cover_course": source_paragraphs[5],
        "cover_subject": source_paragraphs[8],
        "cover_by": source_paragraphs[12],
        "cover_author": source_paragraphs[8],
        "heading1": source_paragraphs[14],
        "heading2": source_paragraphs[15],
        "heading3": source_paragraphs[39],
        "body": source_paragraphs[16],
        "caption": source_paragraphs[128],
        "reference": source_paragraphs[189],
        "table": doc.tables[0],
    }
    ...
    try:
        doc.save(OUTPUT)
        print("Saved complete Word document to:", OUTPUT)
    except PermissionError:
        fallback = OUTPUT.with_name("Finals_Complete_Paper_WASD_updated.docx")
        doc.save(fallback)
        print(f"Warning: {OUTPUT.name} is currently open in Word. Saved to fallback: {fallback}")
