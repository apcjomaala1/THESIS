...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test the trained RoBERTa model.")
    parser.add_argument("--model-dir", type=Path, default=script_dir / "final_roberta_model")
    ...
    print(f"Testing RoBERTa on: {'CUDA' if device == 0 else 'CPU'}")
    if args.text:
        classify(args.text)
        return
    history = []
